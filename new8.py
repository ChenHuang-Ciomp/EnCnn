# -*- coding: utf-8 -*-
"""
Created on Mon Mar 17 09:51:16 2025
...
class SimpleSpectralResNet(nn.Module):
    def __init__(self):
        super(SimpleSpectralResNet, self).__init__()
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, padding=1)  # 保持输入为 (B, 1, 16, 16)
        self.bn1 = nn.BatchNorm2d(16)
        
        # 2个残差块
        self.resblock1 = SimpleResNetBlock(16, 32)
        self.resblock2 = SimpleResNetBlock(32, 64)
...
class SpectralDataset(Dataset):
    def __init__(self, sample_path, g_matrix_path, augment_factor=10):
        ...
        for i in range(augment_factor):
            ...
            augmented_data.append(self.data * scale + torch.randn_like(self.data) * 0.02)
...
# 使用调整后的网络模型
model = SimpleSpectralResNet().to(device)

# 定义损失函数和优化器
criterion = nn.MSELoss()
optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-5)  # 降低学习率
...
# 保存训练好的模型
torch.save(model.state_dict(), 'enhanced_spectral_resnet.pth')
...
# 训练完后加载模型
loaded_model = SimpleSpectralResNet().to(device)
loaded_model.load_state_dict(torch.load('enhanced_spectral_resnet.pth'))
loaded_model.eval()
